...
function calculateAge(birthYear, year) {
  const years = Math.abs(year - birthYear);
  const notPlural = years === 1 ? "year" : "years";
  if (birthYear === year) return "You were born this very year!";
  return `You ${
    birthYear > year
      ? `will be born in ${years} ${notPlural}.`
      : `are ${years} ${notPlural} old.`
  }`;
}
